...
def build_file(dump1):
    ...
    for i in dump1.processes:
        Node=ET.SubElement(data, 'Proc', name=i.name,pid=i.pid,memory_used_by_modules=str(i.memory_used_by_modules),memory_used_by_pages=str(i.memory_used_by_pages))
        for j in i.modules:
